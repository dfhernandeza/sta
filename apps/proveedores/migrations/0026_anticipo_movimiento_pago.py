...
def vincular_movimientos_legacy(apps, schema_editor):
    Anticipo = apps.get_model('proveedores', 'Anticipo')
    MovimientoBancario = apps.get_model('tesoreria', 'MovimientoBancario')

    usados = set()
    for anticipo in Anticipo.objects.filter(
        origen='operacional',
        movimiento_pago__isnull=True,
    ).select_related('proveedor'):
        descripcion = (
            f'Anticipo {anticipo.proveedor.razon_social} - {anticipo.fecha}'
        )
        candidatos = list(
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('proveedores', '0025_aplicacionanticipoproveedor'),
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.OneToOneField(
                ...
                related_name='anticipo_proveedor',
